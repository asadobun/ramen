...
$webshare_token = "63cd8a4dd478ffe315dd5667448d35f068e34ff6"; 
$prox = curl_init();
...
if (curl_errno($ch1)) {
    echo '<div class="dead" style="display:none;">
            <span class="badge badge-danger">DEAD</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe Webshare API | Ramen 🍜 ➢ Connection Timeout</span>
            </div>';
    exit();
}
curl_close($ch1);
...
if(isset($res1['id'])){
    ...
    curl_setopt($ch2, CURLOPT_POSTFIELDS, "email=$email&description=Ramen&source=".$res1["id"]);
...
if(isset($res1['error'])){
    ...
    if($code == "incorrect_cvc"||$decline_code == "incorrect_cvc"){
        ...
        echo '<div class="live_ccn" style="display:none;">
            <span class="badge badge-warning">CCN LIVE</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe Webshare API | Ramen 🍜 ➢ '.$err.'</span>
            </div>';
        ...
        echo '<div class="live_cvv" style="display:none;">
            <span class="badge badge-primary">CVV LIVE</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe Webshare API | Ramen 🍜 ➢ '.$err.'</span>
            </div>';
        ...
        //TESTMODE CHARGES
        echo '<div class="dead" style="display:none;">
            <span class="badge badge-danger">DEAD</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe Webshare API | Ramen 🍜 ➢ Token Error</span>
            </div>';
    }elseif($res1['error']['type'] == "invalid_request_error"){
        //TESTMODE CHARGES
        echo '<div class="dead" style="display:none;">
            <span class="badge badge-danger">DEAD</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe Webshare API | Ramen 🍜 ➢ Invalid SK Provided</span>
            </div>';
    }elseif(strpos($curl1, 'Sending credit card numbers directly to the Stripe API is generally unsafe.')) {
        //VERIFY NUMBER
        echo '<div class="dead" style="display:none;">
        <span class="badge badge-danger">DEAD</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe Webshare API | Ramen 🍜 ➢ '.$err.'</span>
        </div>';
    ...
    if (isset($res2['error'])) {
        ...
        if (isset($res2['sources'])) {
            ...
            if($cvc_res2 == "pass"||$cvc_res2 == "success"){
                //CVV MATCH CONGRATS
                echo '<div class="live_cvv" style="display:none;">
                <span class="badge badge-primary">CVV LIVE</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe Webshare API | Ramen 🍜 ➢ cvc_check : '.$cvc_res2.'</span>
                </div>';
            }else{
                //DEAD
                echo '<div class="dead" style="display:none;">
                <span class="badge badge-danger">DEAD</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe Webshare API | Ramen 🍜 ➢ cvc_check : '.$cvc_res2.'</span>
                </div>';
            }
        }else{
            ...
            if($cvc_res3 == "pass"||$cvc_res3 == "success"){
                //CVV MATCH CONGRATS
                echo '<div class="live_cvv" style="display:none;">
                <span class="badge badge-primary">CVV LIVE</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe Webshare API | Ramen 🍜 ➢ cvc_check : '.$cvc_res3.'</span>
                </div>';
            }else{
                //DEAD
                echo '<div class="dead" style="display:none;">
                <span class="badge badge-danger">DEAD</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe Webshare API | Ramen 🍜 ➢ cvc_check : '.$cvc_res3.'</span>
                </div>';
            }
        }
    }
