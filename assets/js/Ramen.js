...
function checkCards() {
    ...
    check_line.forEach(function(value, index) {
        setTimeout(
            function(){
                $.ajax({
                    url: apis + '?cc_info=' + value + '&sk=' + sk_live + '&referrer=Ramen',
                    type: 'GET',
                    async: true,
...
function checkSK(){
    ...
    setTimeout(
        function(){
            $.ajax({
            url: 'skcheck.php?sk=' + sk_live + '&referrer=Ramen',
            type: 'GET',
            async: true,
            beforeSend: function () {
            ...
            success: function(data){
                if (data.match("LIVE")) {
                    ...
                    $('#ModalMsg').text("SK provided is DEAD. Contact stripe for more details.");
                    playError();
                }
            }
        });
...
function credits(){
    if(!$("#footer").length){
        $("#container").append('<div class="footer" id="footer"><center><p style="color: #FF0000">SK CC Checker</p></center></div>');
    }else{
    	var x = document.getElementById("footer").textContent;
    	if (x != 'Ramen') {
    		$("#container").append('<div class="footer" id="footer"><center><p style="color: #FF0000">Checker ni Ramen</p></center></div>');
    	}
    }
}
